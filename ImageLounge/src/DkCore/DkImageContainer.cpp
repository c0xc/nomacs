...
QImage DkImageContainer::image() {
	...
	return mLoader->lastImage(); //current image excluding rotated pixmaps which represent metadata rotation...
}
...
void DkImageContainer::setImage(const QImage& img, const QString& editName, const QString& filePath) {
	...
	getLoader()->setImage(img, editName, filePath); //TODO this overload is pointless. get rid of filePath. it's already there.
	mEdited = true;
}
